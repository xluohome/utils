...
/**
 * 因为 hoisie/redis有几个方法，不是public的，而且自己另外还需要一些新的没有实现，所以把这个整个移植过来了
 */
/******************begin github.com/hoisie/redis****************************/
import (
    "bufio"
    "bytes"
    "errors"
...
/******************end github.com/hoisie/redis****************************/

type Redis struct {
    pools    map[string]*Client // redis池
    hash     *ConsistentHash    // 一致性哈希算法
    r        *Client            // 不需要一致性的时候的Reids实例
    poolSize int
}

func NewRedis(poolSize int) *Redis {
    return &Redis{pools: make(map[string]*Client), hash: NewConsistentHash(64), poolSize: poolSize}
}

// 加入一个redis服务器
func (this *Redis) AddServer(name, server, password string, db int) error {
    if server == "" {
        return errors.New("can't add nil server")
    }

    client := &Client{Addr: server, Password: password, Db: db, MaxPoolSize: this.poolSize}
    // 检查连接是否正常
    str, err := client.sendCommand("PING")
    if err != nil || str != "PONG" {
        return errors.New("connect redis server falt")
    }

    // 计算服务器的hash值，以便做一致性
    name = Md5Sum(fmt.Sprintf("%s%s%d", name, server, time.Now().UnixNano()))
    // 加入缓存集群中
    this.pools[name] = client
    // 加入到一致性hash中
    this.hash.AddTarget(name)

    return nil
}

// 获取一个redis链接
func (this *Redis) GetConn(key string) *Client {
    if this.r != nil {
        return this.r
    }
    ...
    target := this.hash.LookupTarget(key)
    return this.pools[target]
}

// 获取一个字增长的值
func (this *Redis) Incr(key string, increment ...int64) int64 {
    var e error
    var value int64
    if len(increment) > 0 {
        value, e = this.GetConn(key).Incrby(key, increment[0])
    } else {
        value, e = this.GetConn(key).Incr(key)
    }
    if e != nil {
        LogInfo.Write("get [%s] incr value error:%s", key, e.Error())
        return 0
    }
...
// 设置一个值
func (this *Redis) Set(key, value string, ttl ...int64) bool {
    var e error
    if len(ttl) > 0 {
        e = this.GetConn(key).Setex(key, ttl[0], []byte(value))
    } else {
        e = this.GetConn(key).Set(key, []byte(value))
    }
    if e != nil {
        LogInfo.Write("SET[key:%s][value:%s]error:%s", key, value, e.Error())
        return false
    }
    return true
}
...
// 获取一个值
func (this *Redis) Get(key string) (string, bool) {
    value, e := this.GetConn(key).Get(key)

    if e != nil {
        LogInfo.Write("GET[key:%s]error:%s", key, e.Error())
        return "", false
    }
...
// 删除
func (this *Redis) Del(key string) bool {
    b, e := this.GetConn(key).Del(key)
    if e != nil {
        LogInfo.Write("DEL[key:%s]error:%s", key, e.Error())
        return false
    }
...
// 加入一个hash值
func (this *Redis) HSet(hash, key, value string) bool {
    b, e := this.GetConn(hash).Hset(hash, key, []byte(value))
    if e != nil {
        LogInfo.Write("HSET[hash:%s][key:%s][value:%s]error:%s", hash, key, value, e.Error())
        return false
    }
...
// 添加多个hash值
func (this *Redis) HMSet(hash string, m map[string]interface{}) bool {
    e := this.GetConn(hash).Hmset(hash, m)
    if e != nil {
        LogInfo.Write("HMSET[hash:%s][kv:%v]error:%s", hash, m, e.Error())
        return false
    }
...
// 获取一个hash的一个key值
func (this *Redis) HGet(hash, key string) string {
    value, e := this.GetConn(hash).Hget(hash, key)
    if e != nil {
        LogInfo.Write("HGET[hash:%s][key:%s]error:%s", hash, key, e.Error())
        return ""
    }
...
// 获取hash中的多个key值
func (this *Redis) HMGet(hash string, keys ...string) map[string]string {
    var params []string
    params = append(params, hash)
    client := this.GetConn(hash)
    res, e := client.sendCommand("HMGET", append(params, keys...)...)
    data := make(map[string]string)
    if e != nil {
        LogInfo.Write("HMGET[hash:%s][keys:%v]error:%s", hash, keys, e.Error())
        return data
    }
    d := res.([][]byte)
    if d == nil || len(d) == 0 {
...
// 获取hash中的所有值
func (this *Redis) HGetAll(hash string) map[string]string {
    d := make(map[string][]byte)
    data := make(map[string]string)
    e := this.GetConn(hash).Hgetall(hash, d)
    if e != nil {
        LogInfo.Write("HGETALL[hash:%s]error:%s", hash, e.Error())
        return data
    }
...
// 删除hash中的一个key值
func (this *Redis) HDel(hash, key string) bool {
    b, e := this.GetConn(hash).Hdel(hash, key)
    if e != nil {
        LogInfo.Write("HDEL[hash:%s][key:%s]error:%s", hash, key, e.Error())
        return false
    }
...
// 删除多个hash中的key值
func (this *Redis) HMDel(hash string, keys ...string) bool {
    var params []string
    params = append(params, hash)
    data, e := this.GetConn(hash).sendCommand("HDEL", append(params, keys...)...)
    if e != nil {
        LogInfo.Write("HDEL[hash:%s][keys:%v]error:%s", hash, keys, e.Error())
        return false
    }
